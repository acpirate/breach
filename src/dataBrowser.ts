// Alpha 0.1.0 §5.3 / Alpha 0.3.0 §4.1 — BROWSER file-acquisition adapter. The
// CSV datasets are packaged as application resources via Vite `?raw` imports
// (bundled at build time, served in dev); the shared pure-TypeScript pipeline
// (logic/data/load) receives the same raw text Node reads from disk. Loaded
// once at startup — no reload or hot-swap during a session; data changes
// require a full application reload.
